...
class AudioGenerationService:
    ...
    def generate_audio_for_document(self, db: Session, document_id: int):
        """
        Gera áudio para todos os segmentos de texto de um documento que ainda não foram processados.
        """
        try:
            ...
            if document.status not in [ProcessingStatus.TEXT_EXTRACTED, ProcessingStatus.FAILED]:
                 logger.warning(f"A geração de áudio para o documento {document_id} foi solicitada, mas o seu estado é '{document.status.value}'. O ideal é que esteja em 'text_extracted'.")

            crud_document.update_document_status(db, document_id, ProcessingStatus.PROCESSING)
            logger.info(f"Iniciando geração de áudio para o Documento ID {document_id}.")
            
            self._process_segments(db, document)

            crud_document.update_document_status(db, document_id, ProcessingStatus.COMPLETED)
            logger.info(f"Geração de áudio para o Documento ID {document_id} concluída.")

        except Exception as e:
            logger.error(f"Falha na geração de áudio para o Documento ID {document_id}: {e}", exc_info=True)
    ...
    def _process_segments(self, db: Session, document: Document):
        ...
        segments_to_process = [seg for seg in document.segments if not seg.s3_url]

        if not segments_to_process:
            logger.info(f"Não há segmentos para processar para o Documento ID {document.id}. A marcar como concluído.")
            return

        for segment in segments_to_process:
            logger.info(f"A processar o segmento {segment.segment_index} para o Documento ID {document.id}...")
            ...
            crud_audio_segment.update_segment_url(db, segment_id=segment.id, s3_url=audio_url)
            logger.info(f"Segmento {segment.segment_index} guardado com sucesso. URL: {audio_url}")
